 ...
 * (and optionally TURNKEY_MAPS_KEY) in Netlify → Site settings → Environment
 * variables to override the committed config.js. If any of the required three
 * are missing, the committed config.js is left as-is rather than being
 * overwritten with blank values.
 */
...
const mapsKey = process.env.TURNKEY_MAPS_KEY || ''
...
console.log(`Wrote config.js (backend: ${backendUrl}) (supabase configured)`, mapsKey ? '(maps configured)' : '(no maps key set)')
