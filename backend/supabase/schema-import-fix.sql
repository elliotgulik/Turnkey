-- Fixes a real bug in run_import() (schema-import-servicem8.sql): the
-- invoices insert wrote row_data->>'payment_status' straight through, which
-- is SQL NULL for every source except ServiceM8 (the generic import wizard
-- never sends that key at all). invoices.payment_status is `not null
-- default 'draft'` (schema-calendar-and-checklists.sql) — an explicit NULL
-- insert does NOT fall back to the column default in Postgres, so every
-- invoice row from Jobber/Housecall Pro/Tradify/Fergus/Generic imports threw
-- a not-null-violation, silently caught by the per-row exception handler
-- and reported as an import error. Customers and jobs from those same
-- sources were never affected — only invoices.
--
-- Run this AFTER schema-import-servicem8.sql (it supersedes the same
-- run_import() function). Safe to re-run: idempotent. The rest of that
-- function is unchanged — this is a single coalesce() added to one insert.
...
begin
  ...
  loop
    begin
      ...
      -- THE FIX: coalesce to 'draft' — previously this was a bare
      -- nullif(...), which stays SQL NULL for every non-ServiceM8 source
      -- (they never send a payment_status at all) and fails the column's
      -- NOT NULL constraint on every single invoice row.
      ...
      values (
        ...
        coalesce(nullif(trim(row_data->>'payment_status'),''), case when coalesce((row_data->>'paid')::boolean,false) then 'paid' else 'draft' end),
